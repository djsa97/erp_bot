...
# =========================================
# BYPASS SSL (útil en algunos entornos Mac)
# =========================================
...
# Solo mostrar vendedoras reales
vendedoras_validas = sorted(
    [v for v in df["Vendedora"].dropna().astype(str).str.strip().unique().tolist() if v and v.lower() != "sin dato"]
)
...
# Excluir de base las filas sin vendedora real
...
with col_b:
    ...
    delta10["Magnitud"] = delta10["Diferencia"].abs()
...
ventas_cliente_actual = (
    actual.groupby(["Cliente_normalizado", "Cliente"], as_index=False)["Total producto"]
    .sum()
    .sort_values("Total producto", ascending=False)
...
# Recuperar un nombre visible del cliente
nombres_actual = actual.groupby("Cliente_normalizado", as_index=False)["Cliente"].first()
nombres_prev = anterior.groupby("Cliente_normalizado", as_index=False)["Cliente"].first()
...
comparacion_clientes = comparacion_clientes.merge(
    nombres_prev.rename(columns={"Cliente": "Cliente_prev"}),
    on="Cliente_normalizado",
    how="left"
)

comparacion_clientes["Cliente"] = comparacion_clientes["Cliente"].fillna(comparacion_clientes["Cliente_prev"])
